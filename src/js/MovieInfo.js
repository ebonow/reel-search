...
const MovieInfo = (props) => {
    ...
    const movieInfoCss = "movie-info " + (!isOpen ? '--hidden' : ''); 
    
    return (
        <section className={movieInfoCss}>
            <header className="movie-info-header">
                <h2>{title} ({year})</h2>
                <button className="movie-info-dismiss-button" aria-label="Close" onClick={onClose}>✖️</button>
            </header>
